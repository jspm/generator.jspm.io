...
export async function parseUrlTarget(targetStr) {
    ...
    if (isRelative(targetStr) || registryIndex !== -1 && supportedProtocols.includes(targetStr.slice(0, registryIndex))) {
        ...
        const alias = (pkgUrl ? await resolver.getPackageConfig(pkgUrl) : null)?.name || target.pathname.split('/').pop();
        if (!alias)
...
export function pkgUrlToNiceString(pkgUrl) {
...
export async function toPackageTarget(targetStr, parentPkgUrl) {
...
export function newPackageTarget(target, parentPkgUrl, depName) {
